...
1. **Read the message**: `bun scripts/read-mail.ts --message-id "<id>"` (prints the message as markdown). Only if the decision is still unclear from this message alone, pull thread context: `bun scripts/read-mail.ts --conversation-id "<conversationId>" --top 3 --json` and read the latest exchanges.
