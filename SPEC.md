...
9. **Durability under long runs comes from structural checkpoints, not context self-estimation.** A large inbox is exactly the "run for hours, don't silently degrade" case, and the plugin never asks the orchestrator to watch its own token budget (a skill can't measure that; the harness handles compaction). It stays durable by construction: the per-run + per-email state machine persists every position (resume-safe: `state.ts list-runs` surfaces a dead run and `resume` continues it, never a blind restart), the heavy reads live in per-thread/per-email sub-agents fed isolated bundles (never the orchestrator's context), and the KB queue drains per email. The orchestrator's only discipline is to honor those seams: work in the configured batches, keep document/thread reads in sub-agents, and lean on the checkpoints rather than holding the whole inbox in context.
...
19. **CLI-only Microsoft access** — DECIDED: the plugin uses the CLI at maximum and recodes nothing; it holds NO Graph client and NO token. The one missing write, `create-reply-draft` (Graph `createReplyAll`), is added to ask-marcel-office-cli before milestone M6 (v2.0.0 renamed the binary to `ask-marcel-office` but does not ship it yet). Rationale: the CLI is the single authenticated surface with exactly the right write policy; the old plugin's token-cache coupling dies. Re-evaluated 2026-07-04 against importing ask-marcel-office-cli as a TypeScript library (it exports a full programmatic surface incl. createGraphClient): REJECTED - (a) the draft-gate/audit hooks enforce safety by matching Bash invocations, and library calls are invisible to them; (b) the read-mostly guarantee is a property of the CLI's command surface, while the library exposes a raw send-capable Graph client; (c) agents/skills must shell the binary anyway, so a library path would split access across two diverging versions; (d) the package's runtime deps (playwright, mammoth, jszip) would enter the plugin tree; (e) spawn overhead (~50ms) is noise against Graph latency, and the adapter behind the CommandRunner port would be the same size either way.

    **Re-re-evaluated 2026-07-05 (2.0.0 published locally + P0 spike): PARTIALLY REVERSED → READS-ONLY LIBRARY.** Evidence flipping the calculus: point (e) was wrong by ~80x — measured spawn overhead is **~3.5–4s per call** (the CLI reloads a 1.2MB bundle incl. playwright/xlsx/mammoth + re-auths every invocation), while warm in-process library calls (one reused `graph`) are **~0.5s → ~8x faster**; `fetch-email-bundle` spawns ~20×/email, so the spawn version is ~80s/email vs ~10s via the library. `buildDeps({})` defaults `cachePath` to the CLI's own `~/.ask-marcel/token-cache.json`, so **auth is free** (spike returned the real Graph user); commands run as `commands[name].execute(graph, params)` → `Result<data>` (same `Result` type, no envelope). **Safety preserved by scoping to READS:** the new `Office` adapter proxies only an allowlist of read commands and **never exposes `buildDeps().graph`** (the raw send-capable client); ALL writes/drafts (`create-reply-draft` etc.) stay on the CLI so the Bash-matching draft-gate/audit hooks keep enforcing never-send/approval — this answers (a) and (b). (c) dual-version drift accepted (binary stays for writes/skills; both pinned to the same version; doctor checks both). (d) dep tax confirmed real (**+~60M node_modules**, playwright/xlsx/mammoth JS into the tree; no browser binaries — Bun skips the postinstall). **Plan:** pilot the reads-only swap on `fetch-email-bundle` (`CommandRunner`→`Office` port; base64→`BinaryWriter` for `get-mail-attachment`, replacing the Img-A `--output-path` spawn); `CommandRunner` stays for qmd/bun. **P0 = GO; P1 (`bun add` + seams) pending.**

    **Further strengthened 2026-07-05 (user directive) → LIBRARY-ONLY, NO CLI AT ALL (supersedes the reads-only compromise above).** All M365 access — plugin code, skills, and login — goes through the imported library; the `ask-marcel-office` binary is never spawned and may be uninstalled. Never touch the raw `buildDeps().graph`; only `commands[].execute`. Codified as binding hard rules **R1–R4 in §15.1**. Draft-approval moves to the code state-machine gate (the Bash `draft-gate` hook is inert without a CLI). Scope is now the full migration (all use-cases + skills + setup/login), with `fetch-email-bundle` still the first pilot slice.

    **✅ LANDED 2026-07-06.** The full migration shipped: the `Office` port + adapter (`createOfficeFromRegistry`/`createOffice`, `commands[].execute` only, raw graph never exposed), `runLogin` via `makeLoginAuth`, `BinaryWriter` + `decodeBase64` for attachment bytes. Every M365 use-case (fetch-email-bundle, scan-inbox, extract-voice-corpus, seed-kb, run-doctor) and script left `CommandRunner` (now qmd/bun only). The doctor dropped the obsolete binary `--version` check; a live `get-current-user` is the M365 readiness probe. All four rules are enforced: **R1** (no `ask-marcel-office` literal in src/scripts — eslint `no-restricted-syntax`), **R2** (adapter boundary), **R3** (library import only in composition + `infra/office.ts` — eslint `no-restricted-imports`), **R4** (skills/agents call `bun scripts/*.ts` — `scripts/check-r4-fence.sh`, pre-commit gate 3; the hook is now 9 gates).
20. **No privacy capture filter in v0.1** — DECIDED: all triaged content is capturable to the KB; gardener + manual review are the safety net (deferred, cheap to add later — §13). **Superseded 2026-07-11: SHIPPED.** `data/profile/never-capture.txt` (one literal per line, any language, # comments) is enforced at both capture sinks: kb-queue append refuses a matching candidate (`blocked-by-never-capture`, the researcher records it as a gap) and write-kb-page refuses a page whose NEW content matches (an existing page already naming a term stays mergeable). The list lives in the profile dir — never indexed, never gardened.
...
23. **Any-language operation** — DECIDED (2026-07-04): the plugin must work for any language. Structural signals over string-matching wherever Graph provides them (`@odata.type` for calendar responses); multilingual token tables (data-driven, easy to extend) where text must be matched (chain-cutting headers/date-intros: EN FR DE ES IT PT NL PL SV RU + ZH JA KO exact tokens; calendar-prefix fallback for 7 languages); CJK-aware counting (substantive = 15+ words OR 25+ CJK chars); unicode slugs with deterministic hash fallback (王伟 stays 王伟, symbol-only names never yield an empty path). KB reserved headings stay English as identifiers (principle 8). Anti-style blocks are per-language capable (substring matching works for CJK literals); the shipped catalog is EN-by-design with judgment guidance for other languages. Agents are told emails may be in any language and to reason in the user's primary language. **CJK verified 2026-07-11** (qmd 2.5.2, live probe): BM25 `qmd search` correctly matches ZH person names (王伟), ZH compounds and sub-words (预算审批 / 预算), JA names (田中), and accented FR, with no false positives on absent terms — the parallel-agent search path (BM25-only by design) is CJK-safe. The embedding path (`qmd query` vec/hyde) remains unverified until the GGUF models are pulled; it is only used by the optional main-thread escalation, best-effort.

24. **Embed economy** — DECIDED (2026-07-12): the wrap-up keeps `qmd update` unconditional (BM25 must always reflect the run's KB writes) and runs `qmd embed` best-effort: when it fails because the GGUF models were never pulled, the skill says so once and continues — nothing else degrades, since the only vector consumer is the optional main-thread `qmd query` escalation (§6). No code change; the skills carry the tolerance.
...
| M7 ✅ 2026-07-06 | `kb-lint` + `kb-index-gen` use-cases + the `FileLister` port; the `kb-gardener` skill (Phase 1 auto-apply lint + reindex, Phase 2 plan-then-apply curation); pre-research unattended mode (inbox-zero Phases 0-3 headless, drafting impossible by construction since `user_approved` cannot exist); and scheduling proposed at setup | The recurring machine |
...
### 15.1 Microsoft 365 access — HARD RULES (library-only, no CLI)

Per user directive 2026-07-05, the plugin reaches Microsoft 365 **only** through the imported `ask-marcel-office-cli` TypeScript library. The `ask-marcel-office` **binary is never spawned** and is not a runtime dependency (it may be uninstalled). These are binding hard rules, lint-enforced once the migration removes the last spawn:

- **R1 — No CLI binary, ever.** Nothing spawns `ask-marcel-office` — not code (no `CommandRunner.run('ask-marcel-office', …)`), not scripts, not skills, not setup/login. `CommandRunner` survives only for non-M365 tools (`qmd`, `bun`).
- **R2 — Only curated commands; never the raw Graph.** All Graph access is `commands[name].execute(graph, params)`. Never call `graph.*` (raw HTTP) directly; the raw `graph`/`buildDeps()` is confined to `src/composition/**` and the single `Office` adapter (`src/infra/office.ts`) and is *passed* to `execute`, never invoked. The command registry has no `send`, so it is the only reachable operation set — the read-mostly / never-send guarantee is a property of this boundary. A missing operation is added as a command to the library upstream, never worked around with raw Graph.
- **R3 — Imports fenced.** `ask-marcel-office-cli` is imported only in `src/composition/**` and `src/infra/office.ts` (`no-restricted-imports`). Login uses `buildDeps().makeLoginAuth()` (browser flow via bundled playwright; one-time `playwright install` of browser binaries).
- **R4 — Skills call scripts, not the binary.** Every skill reaches M365 through a `bun scripts/*.ts` entry that uses the library — never a bash `ask-marcel-office` command.

**Consequences:** (i) draft-approval enforcement moves from the now-inert Bash-matching `draft-gate` hook to the **code state-machine gate** (M6); (ii) no dual-version drift (no binary to sync); (iii) the dependency tax (+~60M: playwright/xlsx/mammoth) is accepted; (iv) enforcement lint activates when the last spawn is removed. **Supersedes the "reads-only" compromise in decision 19.**
