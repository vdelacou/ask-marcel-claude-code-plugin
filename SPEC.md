...
| M4 ✅ 2026-07-04 | CLI interlude: `create-reply-draft` landed in ask-marcel-office-cli (commits a2b28da + becf4db, v2.1.0, through that repo's gates; 4388 tests). Live-proven: real threaded reply-all draft created on the PROJECT ORBIT thread (inherited RE: subject + 5 recipients, isDraft guard passed). npm publish + doctor-gate flip to ≥2.1.0 pending Vincent's publish | The only cross-repo dependency, done before it blocks |
| M5 ✅ 2026-07-06 | Research pipeline, all green (≥90% Stryker), all on the Office library (decision 19): `fetch-email-bundle` (thread + attachments + images + SP-B2 SharePoint download), `read-doc` (markdown with a scrambled→PDF fallback), the search module (§6 — parallel kb/mail/sharepoint, all three shapes verified against live qmd + Graph), the `email-researcher` agent, and the KB candidate queue. Img-B (doc-embedded images) deferred as low-value enrichment | Phase 3 works headless |
