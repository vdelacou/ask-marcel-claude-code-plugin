...
import { extractSharepointLinks, isResolvedLink } from './sharepoint-links.ts';
...
describe('extractSharepointLinks', () => {
  test('splits resolved links (driveId + itemId) from errored ones, defaulting missing name and webUrl', () => {
    ...
    expect(extractSharepointLinks(data)).toEqual([
      { url: 'https://x/a', name: 'Spec.docx', webUrl: 'https://x/spec', driveId: 'd1', itemId: 'i1' },
      { url: 'https://x/b', name: '(unnamed)', webUrl: 'https://x/b', driveId: 'd2', itemId: 'i2' },
      { url: 'https://x/c', error: 'access denied' },
