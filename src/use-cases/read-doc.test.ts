...
type Overrides = { readonly markdown?: OfficeResp; readonly pdf?: OfficeResp; readonly failWrite?: (path: string) => boolean };
...
describe('read-doc', () => {
  test('a cleanly converting document lands in the bundle as markdown', async () => {
    ...
    expect(result).toEqual({ ok: true, value: { mode: 'markdown', path: 'docs/q3-plan-docx.md' } });
    // it converts by drive + item id, and never falls back to the PDF renderer
    expect(officeLog).toEqual([{ command: 'download-drive-item-as-markdown', params: { driveId: 'd1', itemId: 'i1' } }]);
    expect(written.find((w) => w.path === `${BASE}/docs/q3-plan-docx.md`)?.content).toBe(GOOD_MD);
    expect(logger.calls).toEqual([{ level: 'info', event: 'read-doc', meta: { name: 'Q3 Plan.docx', mode: 'markdown' } }]);
  });
  ...
  test('a scrambled conversion falls back to rendering the PDF pages', async () => {
    ...
    expect(result).toEqual({ ok: true, value: { mode: 'pdf', path: 'docs/q3-plan-docx.pdf' } });
